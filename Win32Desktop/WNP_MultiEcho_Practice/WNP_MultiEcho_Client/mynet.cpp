...
{
	int received;	//현재 받은량
	char *ptr = buf;	//받을 공간 현재 위치
	int left = len;	//남은 량	100 : 
...
DWORD WINAPI RecvThread(LPVOID data)
{
	...
	while (1)
	{
		...
		//================= 가변 데이터 길이 리시브 ====================
		int recvlen;
		nRead = recvn(s, (LPSTR)&recvlen, sizeof(int), 0);	//send가 왔는데 "넌 요만큼 받을 예정이야"
		if (nRead <= 0) break;

		nRead = recvn(s, (LPSTR)tbuf, recvlen, 0); // "이게 그거야"
		if (nRead <= 0) break;
		//==============================================================
